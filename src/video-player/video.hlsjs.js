...
const HlsSourceHandler = {
  canHandleSource(source) {
    if (source.skipContribHlsJs) return ''
    else if (hlsTypeRE.test(source.type)) return 'probably'
    else if (hlsExtRE.test(source.src)) return 'maybe'
    else return ''
  },
  ...
  canPlayType(type) {
    if (hlsTypeRE.test(type)) return 'probably'
    return ''
  }
}

if (Hls.isSupported()) {
  let videojs = require('video.js') // resolved UMD-wise through webpack
  ...
  if (videojs) {
    ...
    console.warn('vue-video-player-vjs: hls.js Couldn\'t find find window.videojs nor require(\'video.js\')')
  }
}
